...
import com.cym5240.pis.MainActivity2;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == IMAGE_EDIT) {
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				// Log.i("datasetdata", data.getData().toString() + "ssdsd");
				profile.setImageBitmap(profile_bitmap); // ?궗吏? ?�??�??�? ?궗吏�?RI�?? ?뿰寃고븯�??
				// Profile changed
				profile_changed = true;
	...
	public void joinAct() {
		...
		try {
			...
			//??���?? 	Global.dumpArray(array);
				...
				deletetemp();
				Intent intent = new Intent(join.this, MainActivity2.class);
				startActivity(intent);
				finish();
			}
